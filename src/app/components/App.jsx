...
import { exportChartImage } from '../../features/size-chart/services/imageExport';
import { useAppStore } from '../../stores/useAppStore';

// 组件导入
import Toolbar from './Toolbar';
import Sidebar from '../../features/categories/components/Sidebar';
import MainContent from '../../features/size-chart/components/MainContent';
...
const App = () => {
  ...
  const setAppState = (updater) => {
    ...
    if ('sizeSettings' in updates) {
      setSizeSettings(updates.sizeSettings);
    }
    if ('categories' in updates) {
      setCategories(updates.categories);
    }
    if ('selectedCategories' in updates) {
      setSelectedCategories(updates.selectedCategories);
    }
    if ('categoryStartValues' in updates) {
      setCategoryStartValues(updates.categoryStartValues);
    }
    if ('chartData' in updates) {
      setChartData(updates.chartData);
    }
    if ('isSettingsOpen' in updates) {
      setSettingsOpen(updates.isSettingsOpen);
    }
    if ('exportPath' in updates) {
      setExportPath(updates.exportPath);
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = async (event) => {
      ...
      if (event.ctrlKey && (event.key === 's' || event.key === 'S')) {
        ...
        console.log(
          'Ctrl+S pressed, selectedCategories:',
          appState.selectedCategories.length,
          'chartData:',
          !!appState.chartData
        );
        ...
        if (appState.selectedCategories.length > 0) {
          ...
          try {
            await exportChartImage({
              ...
              exportPath: appState.exportPath,
              mode: appState.mode,
              filenamePrefix: '尺码表',
            });
  ...
  useEffect(() => {
    ...
    if (selectedCategories.length > 0) {
      try {
        const newChartData = calculateSizeData(
          sizeSettings,
          selectedCategories,
          mode,
          categoryStartValues
  ...
  return (
    <ThemeProvider theme={theme}>
      <AppContainer data-theme={theme.mode}>
        ...
        {/* 状态栏区域 */}
        <StatusBarSection>
          <StatusBar appState={appState} exportStatus={exportStatus} />
        </StatusBarSection>
