...
const CanvasContainer = styled.div`
  ...
  overflow: auto;
  background: ${(props) =>
    props.theme.colors.background.secondary}; /* 使用纯色背景替代透明格子 */
  min-height: 400px;
`;
...
const PreviewPanel = ({ appState }) => {
  ...
  useEffect(() => {
    const handleExportShortcut = (event) => {
      ...
      const { chartData: eventChartData } = event.detail;

      // 使用传递的 chartData 或组件的 chartData
      const dataToUse = eventChartData || chartData;
      handleExportImage(dataToUse);
    };
  ...
  // 导出为图片
  const handleExportImage = async (dataToUse = null) => {
